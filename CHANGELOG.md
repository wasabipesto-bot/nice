...
- Add the same affine middle-digit filter to the CUDA nice-only kernel: cross-end survivors (compacted or not) are tested with `u64` word arithmetic and constant divisors before the multi-limb square and digit scan. Parameters come from `gpu_config::affine_params`, which enables it wherever the cross-end filter runs (base ≤ 64) and both powers are guaranteed six digits. `NICE_CUDA_AFFINE=0` disables it for A/B measurement. Measured on two Vast RTX 3080 hosts (interleaved rounds, 30 s budgets): 1.33 / 1.39 / 1.39 / 1.37 / 1.48x on b40 MSD-strong / b40 MSD-weak / b50 MSD-weak / b50 residue-dense / b52 MSD-weak with a Ryzen 9 5900X host, 1.13-1.21x with a Xeon E5-2630 v4 host; cumulative GPU nice-only v3.4.5 → this branch on the CUDA backend 1.32-1.46x and 1.32-1.71x respectively. The device arithmetic is mirrored on the host and every base's kernel compiles under NVRTC in all define variants; the 19 ignored device tests pass on the RTX 3080.
- Add the affine middle-digit filter to the CubeCL nice-only kernel as well (all three enumeration paths), in 32-bit digit arithmetic so that no backend ever divides a 64-bit value per candidate: a candidate's `s = n mod b³` and `t = (n div b³) mod b³` are derived from the range's enumeration base and its residue with one 64-bit reduction per range, and the six middle digits come out of schoolbook products of base-b digits. Survivors are filtered before they take a compaction-queue slot. `NICE_CUBECL_AFFINE=0` disables it. The device probe tests (lavapipe, WGSL and direct SPIR-V) compare the kernel's survivor sets with a host mirror in every compaction variant with the filter on and off. Measured on a Vast RTX 3080 (`cubecl-cuda`) and an RX 6900 XT (`wgpu<spirv>`, RADV), that first placement — the test inline in the warp-synchronous enumeration loop — was a net loss (0.79-1.0x and 0.75-1.17x by scenario): a plane pays for the test whenever any lane holds a cross survivor, so nearly every iteration. The compacted paths now carry each queued candidate's `(s, t)` and known-digit mask through the queue instead, and the plane-scoped path drains in two stages: the affine test runs on a dense wave of cross survivors, the few that pass compact into a second queue, and the full check runs only on dense waves of those. The cube-scoped (WGSL) path tests at drain time ahead of the check. Everything the filter adds runs only on iterations that drain (a first version that did its reductions and barriers on every producer iteration measured 0.85-0.95x). `NICE_CUBECL_AFFINE_STAGES=1` selects a single stage (survivors checked in place on the drained wave, the CUDA kernel's shape). Same survivor sets in every mode (lavapipe mirror tests under both compilers, and on the RX 6900 XT). Shared memory grows by 16 KB (plane path, two stages) / 8 KB (one stage, or the cube path) only when the filter is on, and the plan turns the filter off where the device's limit cannot hold that. Measured (three interleaved rounds, filter off → two-stage): RTX 3080 `cubecl-cuda` 0.92 / 0.97 / 1.17 / 1.09 / 1.09 and RX 6900 XT `wgpu<spirv>` 0.93 / 0.99 / 0.96 / 1.09 / 1.17 on b40 MSD-strong / b40 MSD-weak / b50 MSD-weak / b50 residue-dense / b52 MSD-weak — a modest net gain on the b50+ windows and a small loss on b40, far from the CUDA kernel's 1.3-1.5x, because this kernel's per-survivor check was not its bottleneck. The one-stage shape is worse on both cards.
- Cut the MSD interval analysis cost 2-3x per node (b40 655→204 ns, b50 830→301 ns, b60 1045→490 ns on one core): endpoint digit extraction now peels `base^e < 2^32` digits per wide division instead of one, the fixed-width masked recursion carries each range's endpoint digit arrays to its children so a binary split computes two new endpoints instead of four, and the Hall check resolves singleton domains by closure before matching only the wider ones. Same rejections, same leaves, same certificates (pinned by tests against the from-scratch recursion). On top of the affine filter this is another 10-20% on CPU nice-only (b50/b52 MSD-weak 7.2→8.6e9 and 4.2→5.1e10 on 4 threads); it also lowers the host-side cost of feeding the GPU pipeline.
- Split the CPU nice-only stride walk into two phases. Phase 1 streams the gap and low-mask tables and compacts the cross-end survivors (offset, `n mod b⁶`, low mask) into per-thread buffers with no data-dependent branch: eight candidates per step under AVX-512, four under AVX2 (in-register prefix sums for the offsets, compress or table-driven permute for the compaction), or an unconditional-store/conditional-increment scalar loop elsewhere, selected once per process (`NICE_SIMD=0|avx2` caps it for A/B runs). Phase 2 runs the affine filter and the seeded nice check on the survivors only. Same candidates, same checks, same results as the single loop (pinned by a test comparing every implementation's survivor stream with the plain walk on production windows of every specialized base). Measured against the previous build on the benchmark scenarios, 4 threads and 1 thread, interleaved: AVX-512 1.30-1.48x, AVX2 1.18-1.32x, scalar 1.13-1.26x on the check-heavy windows (b40/b50/b52 MSD-weak, b50 residue-dense), +5% on b40 MSD-strong. The public benchmark fleet is nearly all AVX2-only hardware (Zen 2/3, Haswell/Broadwell Xeons, 12th/13th-gen Intel), so the AVX2 figure is the one most machines see.
- Lower the CPU MSD recursion floor from 8000 to 4000: with cheaper nodes and cheaper survivors the single-core whole-field optimum on production-weighted windows moved finer (+2-5% at b40/b52/b57, neutral at b50).
